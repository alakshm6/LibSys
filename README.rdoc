...
The app is deployed and running on EC2 instance in AWS.
...
so as to implement permissioning for various pages. For example, a normal user should not be able to access the admin_home page and alike.
...
The preconfigured admin has already been created. 
name: xyz
email: xyz@gmail.com
password: abc
user_type:P
...
* Admin
First, a preconfigured admin is created who can create other admins. An admin can also create new admins after the admin has signed in. 
...
*	Manage Admins
	...
	*	Delete other admins
		...
		up on the screen saying "User cannot be deleted because there is a book checked out under his name".
		Furthermore, an admin will not be allowed to delete himself and a pre configured admin. Attempts to do so will throw eror messages saying "Admins cannot delete themselves".
...
*	Library Management
	*	Add new book
		...
		the status should be set to "Available" only. The system does not default to it. Failure to do so will lead to abnormal behaviour with respect to functions corresponding to the books.
	...
	*	View list of books
		...
		will take to a page which displays the book details and also prompts for the user name to be checked out under. In this case, if the admin is checking out on
		his behalf he needs to enter his own email id. Also the email id being entered here is not validated against its presence/absence in the database.

	*	View the details of a book
		There is no separate link developed for viewing the details of a book. Clicking on the "View details of a book"  will link to books index. Clicking on show
		corresponding to a book will only show the details of a book. This currently does not show the details of who owns the book currently.
	
	*	Edit the details of a book
		Only the admin is allowed to edit the details of a book. The admin needs to update all the required fields but should not modify the isbn field. The status
...
===Testing===
The test cases written for model are present in the test/model folder and the test cases for the controller are present in the test/controller folder. The files are named as user_test.rb and user_controller_test.rb respectively.
